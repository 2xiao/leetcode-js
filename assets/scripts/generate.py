...
# 格式化每一个frame items
def gen_frame_items(df_indexs, df, solutions_path):
    row = df_indexs[0]
    
    problem_id = df.loc[row, "序号"]
    problem_link = "[" + df.loc[row, "标题末尾"] + "](" + df.loc[row, "标题链接"] + ")"

    problem_solution_path = os.path.join(solutions_path, df.loc[row, "文件名"] + ".md")
...
# 根据标题，读表，生成frame 
def gen_frame(problem_titles, solutions_path):
    ...
    for problem_title in problem_titles:
        ...
        if not df_indexs:
            ...
            continue
        res = gen_frame_items(df_indexs, df, solutions_path)
        frame.loc[frame_cout] = res
        frame_cout += 1
    return frame


# 在文章后自动添加表格，根据str字段判断插入位置
    ...
    for i in range(len(lines)):
        ...
        if match:
            ...
            if title_size == "##":
                ...
                frame = gen_frame(problem_titles, solutions_path)
