...
🔴 <font color=#ff334b>Hard</font>&emsp; 🔖&ensp; [`树`](/tag/tree.md) [`深度优先搜索`](/tag/depth-first-search.md) [`广度优先搜索`](/tag/breadth-first-search.md) [`数组`](/tag/array.md) [`二叉树`](/tag/binary-tree.md)&emsp; 🔗&ensp;[`LeetCode`](https://leetcode.com/problems/height-of-binary-tree-after-subtree-removal-queries)
...
2. **层级信息的存储**：
   ...
   - 使用另一个哈希表（`levelsArr`）来存储每一层的最大高度和第二高的节点，对于每一层，我们需要记录：
     - 最大高度（max）
     - 第二高的高度（second）
     - 最大高度节点的值（maxNodeVal）
...
3. **计算树的高度**：
   ...
   - 通过递归方式遍历树的每个节点，在遍历过程中计算每个节点的高度，并更新 `heightMap` 和 `levelsArr`。
...
- **空间复杂度**：`O(n)`，存储每个节点信息的 `heightMap` 和 `levelsArr` 的空间复杂度均为 `O(n)`；
...
var treeQueries = function (root, queries) {
	...
	let levelsArr = new Map();

	// DFS 计算高度并记录每层的最大和第二高
	calculateHeight(root, heightMap, levelsArr, 0);
	...
	for (let num of queries) {
		...
		const { max, second, maxNodeVal } = levelsArr.get(deep);
		// 检查该节点是否为其所在层的最大高度节点
		if (num === maxNodeVal) {
...
var calculateHeight = function (node, heightMap, levelsArr, deep) {
	if (!node) return -1;

	const left = calculateHeight(node.left, heightMap, levelsArr, deep + 1);
	const right = calculateHeight(node.right, heightMap, levelsArr, deep + 1);
	const height = 1 + Math.max(left, right);

	heightMap.set(node.val, [height, deep]);

	if (!levelsArr.has(deep)) {
		levelsArr.set(deep, { max: height, second: -1, maxNodeVal: node.val });
	} else {
		const levelInfo = levelsArr.get(deep);
