...
2. **状态转移方程：**
   ...
   - 若第 `i` 天 **不持有** 股票(即 `dp[i][0]`)，分为两种情况：
     - 第 `i - 1` 天也不持有股票：`dp[i][0] = dp[i - 1][0]`
     - 第 `i - 1` 天持有股票，第 `i` 天卖出：`dp[i][0] = dp[i - 1][1] + prices[i]`
   - 若第 `i` 天 **持有** 股票(即 `dp[i][1]`)，分为两种情况：
     - 第 `i - 1` 天也持有股票：`dp[i][1] = dp[i - 1][1]`
     - 第 `i - 1` 天不持有股票，第 `i` 天买入，由于题目规定只能买入一次，所以之前都没有交易过：`dp[i][0] = -prices[i]`
...
根据上面的代码可以发现，`dp[i][0]` 代表截至第 `i` 天的最大利润，`dp[i][1]` 代表截至第 `i` 天的最低股价。
