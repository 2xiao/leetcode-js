...
LeetCode 第 433 题 "最小基因变化"（Minimum Genetic Mutation）是一道图遍历问题，主要考察如何从起始基因序列转换到目标基因序列，要求在每一步只改变一个字符，并且这个字符的变化必须存在于一个给定的基因库中。

### 题目解析：

- 基因序列由 8 个字符组成，字符只能是 `A`、`C`、`G`、`T`。
- 你需要从起始序列通过一步步变化，最终变成目标序列。
- 每次变化必须满足：
  1. 只能改变一个字符。
  2. 改变后的序列必须在基因库中存在。
- 你需要计算出从起始序列到目标序列所需的最小变化次数，如果无法达到目标，则返回 `-1`。
