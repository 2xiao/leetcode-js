...
class Twitter {
  ...
  getNewsFeed(userId) {
    let res = [];
    const followee = this.followList[userId];
    for (let i = this.postList.length - 1; i >= 0; i--) {
      if (res.length < 10) {
        const [_userId, _tweetId] = this.postList[i];
        if ((followee && followee.has(_userId)) || userId == _userId) {
          res.push(_tweetId);
        }
      } else {
        break;
  ...
  follow(followerId, followeeId) {
    if (this.followList[followerId]) {
      this.followList[followerId].add(followeeId);
    } else {
      this.followList[followerId] = new Set([followeeId]);
    }
  }
  ...
  unfollow(followerId, followeeId) {
    if (this.followList[followerId]) {
      this.followList[followerId].delete(followeeId);
    }
  }
}
```
