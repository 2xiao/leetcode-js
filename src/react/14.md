...
- 由于 `updateQueue` 是一个链表，因此需要遍历链表，依次消费复合当前更新优先级的 `update`；
...
同样，每个调用 `processUpdateQueue` 函数消费 `Update` 的地方，都需要增加一个 `renderLane` 参数。也有两个方法可以消费更新： `updateHostRoot` 和 `updateState`，分别对应了首屏渲染阶段和组件更新阶段。

这里不一一列举改造细节了，详见 [代码](https://github.com/2xiao/my-react/commit/73e3dbe1457c87eb533e71784ab8a37f801e5ead)。

## 3. 实现调度阶段

之前的更新流程中，`scheduleUpdateOnFiber` 函数会从当前节点向上遍历到根节点，然后调用 Render 阶段的入口 `renderRoot` 函数， 构建 Fiber 树、处理新旧虚拟 DOM 树之间的差异、生成更新计划；在`renderRoot` 函数执行完之后又会调用 Commit 阶段的入口 `commitRoot` 函数，将更新同步到实际的 DOM 中。
...
3. **调度任务完成**：无论使用微任务还是宏任务调度，都会在调度任务后，继续执行剩余的同步任务。这通常由 `scheduleMicroTask` 或类似的机制来实现。
...
为了确保在各种不同的环境下都能够正确地安排微任务，并确保任务的及时执行，需要对不同的环境做出兼容和备选，具体实现原理是：
...
- 首先，`markRootUpdated` 函数会将每次更新的 `lane` 存入 `root.pendingLanes` 中，经过三次 `mergeLanes` 后的 `root.pendingLanes` 为 `SyncLane`;
- 接着，调用 `ensureRootIsScheduled` 函数，开始调度阶段，因为此时的 `updateLane === SyncLane`，为同步任务，会用微任务来调度；
- `scheduleSyncCallback` 函数会被执行三次，所以此时的 `syncQueue` 为三个 `renderRoot` 回调函数数组（`[renderRoot, renderRoot, renderRoot]`）;
...
至此，我们就实现了同步调度流程，可以将多个更新合并到一个微任务中执行。
