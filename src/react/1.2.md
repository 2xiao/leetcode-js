...
本节的相关代码可在`git tag v1.2`查看：[https://github.com/2xiao/my-react/tree/v1.2](https://github.com/2xiao/my-react/tree/v1.2)
...
我们先来实现 react 包中的 `creatElement` 和 `jsx` 方法，并实现 react 包的打包流程。
...
执行 `jsx` 方法和 `React.createElement` 方法的返回结果是一种被称为 `ReactElement` 的数组结构，所以我们首先要定义一下 `ReactElement` 的构造函数：
...
从以上示例可以看出， `jsx` 方法接收两个参数，第一个参数 `type` 为组件的 type，第二个参数是其他配置，可能有第三个参数为组件的 `children`，返回一个 `ReactElement` 结构。
...
为了区分生产环境和开发环境，这里再定义一个 `jsxDEV` 方法，唯一的区别是，开发不处理 `children` 参数，方便多做一些额外的检查：
...
新增 `index.ts` 文件，这个文件是 react 包的入口，导出一个对象，包含版本号 `version` 和 `React.creatElement` 方法。其中，`React.creatElement` 方法就是刚才实现的 `jsx` 方法。
...
可以看到，根目录下的 `dist/node_modules/react` 文件夹中出现了 react 包的打包产物：
...
react 包打包完之后，我们可以使用 `pnpm link` 来调试以下打包结果，流程如下图所示：
...
至此，我们就完成了 JSX 方法的开发、打包、调试，相关代码可在`git tag v1.2`查看，地址：[https://github.com/2xiao/my-react/tree/v1.2](https://github.com/2xiao/my-react/tree/v1.2)
